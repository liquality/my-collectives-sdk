import {custom, createWalletClient, encodeFunctionData,SimulateContractParameters, createPublicClient, hexToBigInt, toHex} from "viem";
...
export async function callData(caller: ethers5.providers.Web3Provider, cMetadata: CMetadata, mintParam: MintParam) {
    ...
    var prepared: SimulateContractParameters;

    return import('@zoralabs/protocol-sdk').then(async (module) => {
        const mintClient = module.createMintClient({chain: walletClient.chain!});
        ...
        prepared = await mintClient.makePrepareMintTokenParams({
