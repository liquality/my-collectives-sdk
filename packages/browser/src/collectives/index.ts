import { ADDRESSES, OPERATOR } from "../constants";
import { AppConfig } from "../config";
import {
  CollectiveFactory__factory,
  Collective__factory,
  createPoolsParam,
  CMetadata,
  JoinCollectiveParam,
...
export class Collective {
  public static async create(
    caller: ethers5.providers.Web3Provider,
    poolParam: createPoolsParam,
    ...
    try {
      ...
      const cWallet = await cFactory.getCWallet(
        cAddress,
        OPERATOR,
        ethers.toBigInt(salt)
      );
      ...
      const initCode = cFactory.interface.encodeFunctionData("createWallet", [
        initiator,
        OPERATOR,
        ethers.toBigInt(salt),
      ]);
      ...
      const factoryAddressBuffer = Buffer.from(
        ethers.toBeArray(cFactory.address)
      );
      ...
      const tx = await buildAndSendUserOperation(
        signer,
        cWallet,
        nonceKey,
        collectiveInitCode,
      ...
      if (tx.status == "success") {
        ...
        tx2 = await this.createColletive(
          signer,
          cFactory,
          nonceKey,
          cAddress,
  ...
  public static async isMember(
    ...
    try {
      requireSupportedChain((await caller.getNetwork()).chainId);
      const signer = caller.getSigner();
      const collective = new ethers5.Contract(
        cMetadata.address,
        Collective__factory.abi,
  ...
  public static async join(
    caller: ethers5.providers.Web3Provider,
    cMetadata: CMetadata,
    JoinParam: JoinCollectiveParam
  ) {
    try {
      requireSupportedChain((await caller.getNetwork()).chainId);
      const signer = caller.getSigner();
      const joinCollectivesCallData = await this.getJoinCollectiveCallData(
        cMetadata.address,
        JoinParam
      );
  ...
  // create a pool
  public static async createPools(
    caller: ethers5.providers.Web3Provider,
    cMetadata: CMetadata,
    poolParam: createPoolsParam
  ) {
    try {
      requireSupportedChain((await caller.getNetwork()).chainId);
      const signer = caller.getSigner();
      const createPoolsCallData = await this.getCreatePoolsCallData(
        cMetadata.address,
        poolParam
      );
      // Create user operation Tx
  ...
  // Get pool
  public static async getPoolByHoneyPot(
    caller: ethers5.providers.Web3Provider,
    cMetadata: CMetadata,
    honeyPot: string
  ) {
  ...
  private static async getCreateCollectiveCallData(
    initiator: string,
    operator: string,
    salt: ethers.BigNumberish
  ) {
    const createCollectiveCallData =
      CollectiveFactory__factory.createInterface().encodeFunctionData(
        "createCollective",
        [initiator, operator, ethers.toBigInt(salt)]
      );
    return createCollectiveCallData;
  }

  private static async getCreatePoolsCallData(
    cAddress: string,
    poolParam: createPoolsParam
  ) {
    const createPoolCallData = new ethers.Interface(
      Collective__factory.abi
    ).encodeFunctionData("createPools", [
      poolParam.tokenContracts,
      poolParam.honeyPots,
    ]);
    return createPoolCallData;
  }

  private static async getJoinCollectiveCallData(
    cAddress: string,
    joinParam: JoinCollectiveParam
  ) {
    const JoinCollectiveCallData =
      Collective__factory.createInterface().encodeFunctionData(
        "joinCollective",
        [joinParam.inviteSignature, joinParam.inviteCode]
      );
    return JoinCollectiveCallData;
  }

  private static async getLeaveCollectiveCallData() {
    const leaveCollectiveCallData =
      Collective__factory.createInterface().encodeFunctionData(
        "leaveCollective"
      );
  ...
  private static async getCFactory(
    runner: ethers5.ethers.providers.JsonRpcSigner
  ) {
    const collectiveFactory =
      ADDRESSES[(await runner.getChainId()) as SupportedChains]
        .collectiveFactory;
    const cFactory = new ethers5.Contract(
      collectiveFactory,
      CollectiveFactory__factory.abi,
      runner
    );
    return cFactory;
  }
  ...
  private static getRemoveMemberCallData() {
    const removeMemberCallData = new ethers.Interface(
      Collective__factory.abi
    ).encodeFunctionData("removeMember");
    return removeMemberCallData;
  }

  private static async createColletive(
    signer: ethers5.providers.JsonRpcSigner,
    cFactory: ethers5.ethers.Contract,
    nonceKey: bigint,
    cAddress: string,
    cWallet: string,
    poolParam: createPoolsParam,
    ...
    try {
      ...
      const createCollectiveCallData = await this.getCreateCollectiveCallData(
        initiator,
        OPERATOR,
        ethers.toBigInt(salt)
      );
