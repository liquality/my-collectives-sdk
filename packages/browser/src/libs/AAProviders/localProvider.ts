...
export class LocalProvider extends BaseProvider {
    ...
    try {
      const entryPoint =
        ADDRESSES[(await this.signer.getChainId()) as SupportedChains]
          .entryPoint;
      const entryPointContract = new ethers5.Contract(
        entryPoint,
        ENTRYPOINT_ABI,
        this.signer
      );
      const estimations =
        await entryPointContract.estimateGas.userOperationGasEstimation(
          userOperation
        );
      return estimations;
    } catch (error) {
  ...
  public async sponsor(userOperation: IUserOperation): Promise<any> {
    try {
      ...
      const estimations =
        await entryPointContract.estimateGas.sponsorUserOperation(
          userOperation
        );
      return estimations;
    } catch (error) {
