...
import {
  ...
  CALL_GAS_LIMIT,
  ENTRYPOINT_ABI,
  USER_OPERATIONS_DEFAULT_SIGNATURE,
} from "../constants";
...
export async function buildAndSendUserOperation(
  signer: ethers5.Signer,
  smartAccount: string,
  nonceKey: bigint,
  collectiveInitCode: BytesLike,
...
export async function buildUserOperation(
  ...
  try {
    ...
    let userOperation: IUserOperation = await InitializeUserOperation(
      smartAccount,
      nonce,
      executeCallData
    );
    ...
    const paymasterAndDataResponse = await userOpsProvider.sponsor(
      userOperation,
      signer
    );
    ...
    if (
      ...
      userOperation.callGasLimit =
        paymasterAndDataResponse.callGasLimit.toString();
      userOperation.verificationGasLimit =
        paymasterAndDataResponse.verificationGasLimit.toString();
      userOperation.preVerificationGas =
        paymasterAndDataResponse.preVerificationGas.toString();
    }
...
async function getNonce(smartAccount: string, nonceKey: bigint) {
  try {
    const entryPointAddress =
      ADDRESSES[
        Number(
          (await AppConfig.PROVIDER.getNetwork()).chainId
        ) as SupportedChains
      ].entryPoint;

    const entryPoint = new ethers5.Contract(
      entryPointAddress,
      ENTRYPOINT_ABI,
      AppConfig.PROVIDER
    );
...
async function getExecuteCallData(transactions: Transaction[]) {
  try {
    ...
    if (transactions.length == 1) {
      executeCallData = CWallet__factory.createInterface().encodeFunctionData(
        "execute",
        [
          transactions[0].to,
          transactions[0].value as BigNumberish,
          transactions[0].data as BytesLike,
        ]
      );
    } else {
...
async function InitializeUserOperation(
  smartAccount: string,
  nonce: string,
...
async function signUserOps(signer: ethers5.Signer, userOpsHash: string) {
  try {
    ...
    const unsignedUserOpsEncoded = ethers5.utils.defaultAbiCoder.encode(
      ["bytes32", "address", "uint256"],
      [userOpsHash, entryPoint, (await AppConfig.PROVIDER.getNetwork()).chainId]
    );
    const unsignedUserOps = ethers5.utils.keccak256(unsignedUserOpsEncoded);
    const signedUserOps = await signer.signMessage(
      ethers5.utils.arrayify(unsignedUserOps)
    );
...
export async function queryReceipt(
  ...
  try {
    ...
    while (receipt === null && Date.now() - startTime < timeOut) {
      ...
      receipt = await rpcCall(bundler, "eth_getUserOperationReceipt", [
        userOpHash,
      ]);
      console.log(
        receipt === null
...
export async function userOpsStatusByHash(
