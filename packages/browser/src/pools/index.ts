...
export class Pool {
  ...
  public static async mint(
    ...
    try {
      ...
      const tokenMintCallData = await this.getTokenMintCallData(
        caller,
        cMetadata,
        mintParam
      );
      ...
      const cWalletContract = new ethers5.Contract(
        cMetadata.wallet,
        CWallet__factory.abi,
        signer
      );
      if (mintParam.amount > 0) {
        const tx = await cWalletContract.depositTo(await signer.getAddress(), {
          value: mintParam.amount,
        });
        await tx.wait();
      }
  ...
  public static async withdrawRewards(
    caller: ethers5.providers.Web3Provider,
    cMetadata: CMetadata,
    pools: string[],
    participant: string
    ...
    try {
      ...
      const withdrawRewardCallData =
        this.getPoolWithdrawRewardCallData(participant);
      const userOpTxs: Transaction[] = [];
  ...
  public static async getPoolParticipants(
    caller: ethers5.providers.Web3Provider,
    pool: string
  ) {
    const poolContract = new ethers5.Contract(
  ...
  private static async getTokenMintCallData(
    caller: ethers5.providers.Web3Provider,
    cMetadata: CMetadata,
    mintParam: MintParam
  ) {
    switch (mintParam.platform) {
      ...
      case SupportedPlatforms.LOCAL:
        return MockTokenContract__factory.createInterface().encodeFunctionData(
          "mint",
          [mintParam.recipient]
        );
      default:
  ...
  private static getRecordPoolMintCallData(mintParam: MintParam) {
    const mintCallData = new ethers5.utils.Interface(
      Collective__factory.abi
    ).encodeFunctionData("recordPoolMint", [
      mintParam.poolAddress,
      mintParam.recipient,
      mintParam.tokenID,
      mintParam.quantity,
      mintParam.amount,
  ...
  private static getPoolDistributeRewardCallData() {
    const distributeRewardCallData =
      Pool__factory.createInterface().encodeFunctionData("distributeReward");
  ...
  private static getPoolWithdrawRewardCallData(participant: string) {
    const withdrawRewardCallData = new ethers.Interface(
      Pool__factory.abi
    ).encodeFunctionData("withdrawReward", [participant]);
